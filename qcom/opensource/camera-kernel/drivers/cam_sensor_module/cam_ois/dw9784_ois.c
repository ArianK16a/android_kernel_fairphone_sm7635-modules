	...
	uint32_t addr, uint32_t *data)
{
	...
	ret = camera_io_dev_read(&(o_ctrl->io_master_info), addr, data, CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_WORD,false);
	if (ret < 0)
		CAM_ERR(CAM_OIS,"err! ret:%d", ret);
	o_ctrl->io_master_info.cci_client->i2c_freq_mode = temp_freq;
...
#else
	printk("dw9784_flash_if_ram_read no need");
